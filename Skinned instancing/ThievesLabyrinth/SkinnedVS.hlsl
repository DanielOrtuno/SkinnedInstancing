...
OUTPUT main(VERTEX input)
{
	...
	for(int i = 0; i < 4; i++)
	{
		AnimationData tCurrAnimData = tAnimationData[input.nInstanceId];

		int nTextureWidth;
		int nTextureHeight;
		AnimPositions.GetDimensions(nTextureWidth, nTextureHeight);

		float fLerpAdjust = ( tCurrAnimData.nFrameCount -1) / ( nTextureWidth + 0.0f );
		float fRatio = tCurrAnimData.fCurrentTime / tCurrAnimData.fTotalTime;

		float2 vUV = float2( fRatio * fLerpAdjust, ( input.vJointIndex[i] + ( nJointCount * tCurrAnimData.nCurrentAnimation ) ) / ( nTextureHeight - .5f ) );
	...
	output.vPosition = mul(vSkinnedPos, modeling[input.nInstanceId]);
	...
	output.vNormal = normalize(mul(vSkinnedNorm, modeling[input.nInstanceId])).xyz;
